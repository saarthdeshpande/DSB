                ...
                if isinstance(d, dict) and d.get("kind") == "Deployment":
                    ...
                    for k in ("containers", "initContainers"):
                        for c in (pod_spec.get(k, []) or []):
                            ...
                            rq['cpu'] = DEF_all['req']['cpu']
                            rq['memory'] = DEF_all['req']['memory']
                            lm['cpu'] = DEF_all['lim']['cpu']
                            lm['memory'] = DEF_all['lim']['memory']
...
def main():
    ...
    try:
        ...
        command = shlex.split(f'wrk -t1 -c400 -d{args.time} -R2000 -s {wrk2file_path} http://{frontend_ip}')
